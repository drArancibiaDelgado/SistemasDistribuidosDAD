...
public class TareaHandler extends Thread {
    final Socket s;
    final DataInputStream dis;
    final DataOutputStream dos;

    public TareaHandler(Socket s, DataInputStream dis, DataOutputStream dos) {
        this.s = s;
        this.dis = dis;
        this.dos = dos;
    }
    ...
    public void run() {
        try {
            while (true) {
                ...
                if (opcion.equals("1")) {
                    if (ServidorTareas.listaTareas.isEmpty()) {
                        dos.writeUTF("La lista está vacía.");
                    } else {
                        String lista = "TAREAS ACTUALES:\n";
                        for (int i = 0; i < ServidorTareas.listaTareas.size(); i++) {
                            lista += i + ". " + ServidorTareas.listaTareas.get(i) + "\n";
                        }
                        dos.writeUTF(lista);
                    }
                } 
                    ...
                    ServidorTareas.listaTareas.add(nuevaTarea.trim());
                    ...
                    try {
                        ...
                        if (index >= 0 && index < ServidorTareas.listaTareas.size()) {
                            String eliminada = ServidorTareas.listaTareas.remove(index);
